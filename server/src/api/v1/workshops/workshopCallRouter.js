...
const { WorkshopCallsService } = require('../../../services');
const { version } = require('../../../config');
const { koaJwt } = require('../../../middlewares');

// Prefix all routes with: /user
const router = new Router({
    prefix : `${version.v1}/workshopCall`,
});
...
router.get('/', async (ctx, next) =>
{
    ...
    const result = await WorkshopCallsService.find();
...
router.get('/edit', async (ctx, next) =>
{
    ...
    const result = await WorkshopCallsService.findToBeApproved();
...
router.post('/', koaJwt, async (ctx, next) =>
{
    ...
    const result = await WorkshopCallsService.create(description);
...
router.put('/', koaJwt, async (ctx, next) =>
{
    ...
    const result = await WorkshopCallsService.approve(description);
