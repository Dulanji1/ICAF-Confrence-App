...
const ResearchPaperPublish = (props) =>
{
	...
	const onSubmit = (e) =>
	{
		...
		if (values.category.length <= 5)
		{
			setAlertData({ display: true, message: 'category must be at least 6 characters long.' });
		...
		{
			setAlertData({ display: true, message: 'title must be at least 6 characters long.' });

			return;
		}

		if (values.publisherName.length <= 0)
		{
			setAlertData({ display: true, message: 'publisher name is required.' });
		...
		communicationService.researchUpload(formData, (res) =>
		{
			setSuccessData({
				display : true,
				...
				<Card>
					...
					<CardContent>
						<div className='mb-3'>
							...
							<br />
							<select
								className='form-control' onChange={(e) => setValues(
									{ ...values, category: e.target.options[e.target.selectedIndex].text },
								)}
							>
...
export default ResearchPaperPublish;
